...
def create_factors_for_bias(num_users, num_items, lda, user_bias = True):
    if user_bias:
        ...
        # Add regularization.
        regularizer = tf.multiply(tf.reduce_sum(tf.square(H)), lda, name="regularize")
...
def learn_item_bias_from_fixed_user_bias(ratings_tr, ratings_val, user_bias, num_items, lda, global_mean, max_iter):
    W = tf.concat([tf.convert_to_tensor(user_bias, dtype=float32, name="user_bias"), tf.ones((user_bias.shape[0],1), dtype=float32, name="item_bias_ones")], 1)
    H = tf.Variable(tf.truncated_normal([1, num_items], stddev=0.02, mean=0), name="items")
    H_with_user_bias = tf.concat([tf.ones((1, num_items), name="user_bias_ones", dtype=float32), H], 0)
    regularizer = tf.multiply(tf.reduce_sum(tf.square(H)), lda, name="regularize")
...
def create_factors_with_biases(user_bias, item_bias, rank, lda):
    ...
    H_plus_bias = tf.concat([H, tf.ones((1, num_items), name="user_bias_ones", dtype=float32), tf.convert_to_tensor(item_bias, dtype=float32, name="item_bias")], 0)
    regularizer = tf.multiply(tf.add(tf.reduce_sum(tf.square(W)), tf.reduce_sum(tf.square(H))), lda, name="regularize")
